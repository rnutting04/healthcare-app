...
import requests
import json
from .middleware_language import UserLanguageMiddleware
    ...
    def _handle_api_auth(self, request):
        ...
        try:
            ...
            if verify_response.status_code == 200:
                ...
                print(f"DEBUG: User role check - Role: '{request.user_role}', Expected: ['PATIENT', 'ADMIN']")
                if request.user_role not in ['PATIENT', 'ADMIN']:
                    print(f"DEBUG: Access denied - User role '{request.user_role}' not in allowed roles")
                    return JsonResponse({'error': 'Access denied. Patients only.'}, status=403)
                
                return self.get_response(request)
            else:
                print(f"DEBUG: Auth verification failed: {verify_response.status_code}")
